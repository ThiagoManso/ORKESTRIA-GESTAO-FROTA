...
import { Settings, Save, MapPin, Building, Loader2, AlertCircle, Clock, Camera } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  // Inspection frequency settings (1 = Todos os dias, 3 = A cada 3 dias, 7 = Semanal)
  const [inspectionFrequencyDays, setInspectionFrequencyDays] = useState('3');
  const [isSavingInspection, setIsSavingInspection] = useState(false);
  const [saveInspectionStatus, setSaveInspectionStatus] = useState<'idle' | 'success' | 'error'>('idle');
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (inspectionSnap.exists()) {
        const iData = inspectionSnap.data();
        if (iData.frequencyDays !== undefined) {
          setInspectionFrequencyDays(iData.frequencyDays.toString());
        }
      }
    };
    fetchSettings();
  ...
  const handleSaveInspection = async (e: React.FormEvent) => {
    ...
    setSaveInspectionStatus('idle');
    try {
      await setDoc(doc(db, 'settings', 'inspection'), {
        frequencyDays: parseInt(inspectionFrequencyDays, 10) || 3
      }, { merge: true });
      setSaveInspectionStatus('success');
      setTimeout(() => setSaveInspectionStatus('idle'), 3000);
    } catch (err) {
      console.error('Error saving inspection frequency:', err);
      setSaveInspectionStatus('error');
    } finally {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 w-full h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-y-auto custom-scrollbar">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden h-fit">
          <div className="p-6 border-b border-slate-100 flex items-center gap-3 bg-slate-50">
            <div className="bg-violet-500/10 text-violet-600 p-2 rounded-lg">
              <Camera size={20} />
            </div>
            <h2 className="text-lg font-bold text-slate-800">Vistoria Fotográfica (Checklist IA)</h2>
          </div>
          
          <form onSubmit={handleSaveInspection} className="p-6 space-y-6">
            <p className="text-sm text-slate-600">
              Defina a periodicidade para exigência obrigatória das 4 fotos (frente, traseira e laterais) e comparativo de gabarito com Inteligência Artificial ao iniciar o dia.
            </p>

            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2.5">Frequência da Vistoria <span className="text-red-500">*</span></label>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  { value: '1', label: 'Todos os dias', desc: 'A cada dia de operação' },
                  { value: '3', label: 'A cada 3 dias', desc: 'Ciclo padrão (3 em 3 dias)' },
                  { value: '7', label: 'Semanal', desc: 'A cada 7 dias (semanal)' }
                ].map((option) => (
                  <button
                    key={option.value}
                    type="button"
                    onClick={() => setInspectionFrequencyDays(option.value)}
                    className={`flex flex-col items-center text-center p-3.5 rounded-xl border-2 transition-all cursor-pointer ${
                      inspectionFrequencyDays === option.value
                        ? 'border-violet-600 bg-violet-50 text-violet-950 font-bold shadow-sm ring-1 ring-violet-600/30'
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
              <div className="text-sm font-medium">
                {saveInspectionStatus === 'success' && <span className="text-emerald-600">Frequência salva com sucesso!</span>}
                {saveInspectionStatus === 'error' && <span className="text-red-600">Erro ao salvar.</span>}
              </div>
              
              <button 
                type="submit"
                disabled={isSavingInspection}
                className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-xl font-semibold hover:opacity-90 transition-opacity shadow-sm disabled:opacity-50"
              >
                {isSavingInspection ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
                Salvar Frequência
              </button>
            </div>
          </form>
