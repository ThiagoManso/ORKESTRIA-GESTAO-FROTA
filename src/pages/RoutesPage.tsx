...
import { collection, addDoc, updateDoc, doc, deleteDoc, writeBatch, getDoc, getDocs, query, where } from 'firebase/firestore';
...
export default function RoutesPage() {
  ...
  const { data: dailyLogs } = useCollection<any>('dailyLogs');
  const [matrizAddress, setMatrizAddress] = useState<string>('Matriz - Av. Paulista, 1000 - São Paulo, SP');
  const getMatrizAddress = () => (matrizAddress && matrizAddress.trim() !== '') ? matrizAddress : 'Matriz - Av. Paulista, 1000 - São Paulo, SP';
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (docSnap.exists() && docSnap.data().address) {
          ...
          setMatrizAddress('Matriz - Av. Paulista, 1000 - São Paulo, SP');
        }
      } catch (error) {
  ...
  const getDriverAvailability = (driverName: string, date: string) => {
    ...
    const driverRoutes = routes.filter(r => (r.driver || '').trim().toLowerCase() === cleanName && r.date === date && r.status !== 'issue');
  ...
  const getOnlineDrivers = () => {
    ...
    const online = rawDrivers.filter((d: any) => {
      if (d.status === 'offline' || d.status === 'inactive' || d.status === 'pending_approval') {
        return false;
      }
      const driverNameClean = (d.name || '').trim().toLowerCase();
      const hasActiveLog = dailyLogs?.some((log: any) => {
        if (log.status !== 'active') return false;
        if (log.driverId && d.id && log.driverId === d.id) return true;
        const logName = (log.driverName || '').trim().toLowerCase();
        return (
          (logName && logName === driverNameClean) ||
          (driverNameClean && logName.includes(driverNameClean)) ||
          (driverNameClean && driverNameClean.includes(logName))
        );
      });
      return hasActiveLog;
    });
  ...
  const fallbackCalculateRouteData = async (routeData: any) => {
    const points: string[] = [
      routeData.origin,
      ...(routeData.intermediates || []).filter((a: string) => a && a.trim() !== ''),
      routeData.destination
    ].filter(Boolean);

    const getCoords = async (address: string, idx: number): Promise<{ lat: number; lng: number }> => {
      try {
      ...
      try {
        const osmRes = await fetch(
          ...
          { headers: { 'User-Agent': 'OrkestriaOS/1.0' } }
        );
      ...
      const latOffset = ((hash % 120) / 1000) * (idx % 2 === 0 ? 1 : -1);
      const lngOffset = (((hash >> 3) % 120) / 1000) * (idx % 3 === 0 ? 1 : -1);
      return {
    ...
    for (let i = 0; i < points.length; i++) {
      const c = await getCoords(points[i], i);
      coords.push(c);
      if (i > 0) {
        totalDistanceMeters += calculateDistance(coords[i - 1].lat, coords[i - 1].lng, c.lat, c.lng);
      }
    }

    const numIntermediates = (routeData.intermediates || []).filter((a: string) => a && a.trim() !== '').length;
    let distanceKm = (totalDistanceMeters / 1000) * 1.35;
    if (distanceKm < 4) {
      distanceKm = 14.5 + numIntermediates * 6.8;
    }

    const drivingMinutes = Math.ceil(distanceKm * 2.15);
    const stopMinutes = numIntermediates * stopTimeMinutes;
    ...
    const newIntermediates = routeData.intermediates || [];
    const newMetadata = routeData.intermediateMetadata || [];
    for (let i = 0; i < newIntermediates.length; i++) {
      if (!newMetadata[i]) newMetadata[i] = {};
      if (coords[i + 1]) {
        newMetadata[i].lat = coords[i + 1].lat;
        newMetadata[i].lng = coords[i + 1].lng;
      }
    }

    const destCoord = coords[coords.length - 1] || { lat: -23.5505, lng: -46.6333 };
  ...
  const calculateRouteData = async (routeData: any) => {
    if (!routeData.origin || !routeData.destination) return { error: 'Preencha a origem e destino.' };
    ...
    if (routesLib) {
      try {
        ...
        console.warn('Google Maps Directions API error, using fallback calculation:', error);
      }
    }

    return fallbackCalculateRouteData(routeData);
  };
  ...
  const calculateRoute = async () => {
    ...
    if (data && !data.error) {
      ...
      const activeDrivers = getOnlineDrivers();
      ...
      for (const d of activeDrivers) {
        ...
        if (d.name === suggestedDriver && avail.minutesLeft >= (data.estimatedMinutes || 0)) {
      ...
      if (!currentDriverHasTime) {
        if (maxMinutesLeft >= (data.estimatedMinutes || 0)) {
  ...
  useEffect(() => {
    if (!isModalOpen || !newRoute.origin || !newRoute.destination || !newRoute.origin.trim() || !newRoute.destination.trim()) {
      return;
    }
  ...
  }, [isModalOpen, newRoute.origin, newRoute.destination, JSON.stringify(newRoute.intermediates || []), newRoute.returnToMatriz]);

  useEffect(() => {
    if (!editingRoute || !editingRoute.origin || !editingRoute.destination || !editingRoute.origin.trim() || !editingRoute.destination.trim()) {
      return;
    }
    const timer = setTimeout(async () => {
    ...
    return () => clearTimeout(timer);
  }, [editingRoute?.origin, editingRoute?.destination, JSON.stringify(editingRoute?.intermediates || []), editingRoute?.returnToMatriz]);
  ...
  const handleAddRoute = async (e: React.FormEvent) => {
    ...
    if (newRoute.driver && newRoute.driver !== 'Aguardando') {
      syncDriverStatus({ name: newRoute.driver }, 'on_route').catch(console.error);
    }
  ...
  const handleEditRoute = async (e: React.FormEvent) => {
    ...
    if (allCompleted && editingRoute.driver) {
      syncDriverStatus({ name: editingRoute.driver }, 'active').catch(console.error);
    } else if (updatedRoute.driver && updatedRoute.driver !== 'Aguardando') {
      syncDriverStatus({ name: updatedRoute.driver }, 'on_route').catch(console.error);
    }
  ...
  const handleDeleteRoute = async (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir esta rota? Os endereços retornarão ao Banco de Demandas.')) {
      ...
      if (routeToDelete && routeToDelete.driver && routeToDelete.driver !== 'Aguardando') {
        syncDriverStatus({ name: routeToDelete.driver }, 'active').catch(console.error);
      }
      await remove(id);
  ...
  const handleManualCompleteStop = async (routeId: string, stopIndex: number) => {
    ...
    try {
      ...
      if (allCompleted && route.driver && route.driver !== 'Aguardando') {
        syncDriverStatus({ name: route.driver }, 'active').catch(console.error);
      }
  ...
  const activeDriversStatus = getOnlineDrivers().map((d: any) => {
    ...
    <div className="p-4 sm:p-6 lg:p-8 w-full h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex-1 flex flex-col overflow-hidden">
                ...
                <tr>
                  <td colSpan={6} className="p-12 text-center text-slate-500">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <span className="font-semibold text-slate-700 text-base">Nenhuma rota encontrada</span>
                      <span className="text-xs text-slate-400 max-w-sm">
                        As rotas cadastradas para a sua empresa aparecerão aqui. Tente limpar os filtros se estiver procurando uma rota específica.
                      </span>
                    </div>
                  </td>
                </tr>
                  ...
                  <tr key={route.id} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-slate-800">{route.driver}</p>
                          <p className="text-xs text-slate-500">{route.vehicle || 'Não atribuído'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-5">
                      <div className="text-sm text-slate-700 font-medium flex items-center gap-1.5">
                        ...
                        <span className="truncate max-w-[200px]" title={route.destination}>{route.destination}</span>
                      </div>
                      <div className="text-xs text-slate-500 mt-0.5 font-medium">
                        {route.stops || 1} parada(s) programada(s)
                      </div>
                    </td>
                    <td className="p-5">
                      <div className="text-sm font-semibold text-slate-700">{route.distance || 0} km</div>
                      <div className="text-xs text-slate-500 font-medium">{route.estimatedTime || '--'}</div>
                    </td>
        ...
        <div className="p-4 sm:p-5 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-slate-500 bg-slate-50/50">
          <span className="font-medium">Mostrando {filteredRoutes.length} de {routes.length} rotas</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1.5">
                      ...
                      <button type="button" onClick={() => setNewRoute({...newRoute, origin: getMatrizAddress()})} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">Usar Matriz</button>
                    </div>
                    <input 
                      type="text" 
                      required
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        setNewRoute({
                          ...
                          destination: checked ? getMatrizAddress() : ''
                        });
                      }}
                      className="w-4 h-4 text-indigo-600 rounded focus:ring-indigo-500"
                    />
                  ...
                  {!newRoute.returnToMatriz && (
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        ...
                        <button type="button" onClick={() => setNewRoute({...newRoute, destination: getMatrizAddress()})} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">Usar Matriz</button>
                      </div>
                      <input 
                        type="text" 
                              ...
                              <div className="mt-2 bg-red-50 border border-red-100 rounded-lg p-3">
                                ...
                                {stop.issuePhotoUrl && (
                                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mt-2">
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => downloadPhotoWithOSName(stop.issuePhotoUrl, stop.orderNumber, stop.externalRequestId, stop.id, 'OCORRENCIA')}
                                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-red-50 text-red-700 border border-red-200 font-semibold text-xs rounded-lg shadow-sm transition-colors"
                                      title="Baixar imagem com número da OS / Pedido"
                                    >
                                      <Download size={14} />
                                      <span>Baixar Foto (OS #{stop.orderNumber || stop.externalRequestId || stop.id?.slice(0, 6)})</span>
                                    </button>
                                  </div>
                                )}
                              </div>
                              ...
                              <div className="mt-2 bg-emerald-50/50 border border-emerald-100 rounded-lg p-3">
                                ...
                                {(stop.podData?.photoUrl || stop.dropoffPhotoUrl) && (
                                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mt-2">
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => downloadPhotoWithOSName(stop.podData?.photoUrl || stop.dropoffPhotoUrl, stop.orderNumber, stop.externalRequestId, stop.id, 'COMPROVANTE')}
                                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-emerald-50 text-emerald-800 border border-emerald-300 font-semibold text-xs rounded-lg shadow-sm transition-colors"
                                      title="Baixar comprovante com número da OS / Pedido"
                                    >
                                      <Download size={14} />
                                      <span>Baixar Comprovante (OS #{stop.orderNumber || stop.externalRequestId || stop.id?.slice(0, 6)})</span>
                                    </button>
                                  </div>
                                )}
                              </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <label className="block text-sm font-semibold text-slate-700">Origem</label>
                        <button type="button" onClick={() => setEditingRoute({...editingRoute, origin: getMatrizAddress()})} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">Usar Matriz</button>
                      </div>
                      <input 
                        type="text" 
                  ...
                  <div>
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          ...
                          setEditingRoute({
                            ...editingRoute, 
                            returnToMatriz: checked,
                            destination: checked ? getMatrizAddress() : editingRoute.destination
                          });
                        }}
                        className="w-4 h-4 text-indigo-600 rounded focus:ring-indigo-500"
                      />
                    ...
                    {!editingRoute.returnToMatriz && (
                      <div>
                        <div className="flex justify-between items-center mb-1.5">
                          <label className="block text-sm font-semibold text-slate-700">Destino</label>
                          <button type="button" onClick={() => setEditingRoute({...editingRoute, destination: getMatrizAddress()})} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">Usar Matriz</button>
                        </div>
                        <input 
                          type="text" 
