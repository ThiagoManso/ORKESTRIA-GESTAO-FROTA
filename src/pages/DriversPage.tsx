...
export default function DriversPage() {
  ...
  const isDriverOnlineAndReady = (driver: any) => {
    if (!driver || driver.status === 'offline' || driver.status === 'inactive' || driver.status === 'pending_approval') {
      return false;
    }
    ...
    const hasActiveLog = dailyLogs?.some((log: any) => {
      ...
      if (log.driverId && driver.id && log.driverId === driver.id) return true;
      const logName = (log.driverName || '').trim().toLowerCase();
      return logName && driverNameClean && (logName === driverNameClean || driverNameClean.includes(logName) || logName.includes(driverNameClean));
    });
    return Boolean(hasActiveLog);
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 w-full h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {[
          ...
          { label: 'Online / Disponíveis', value: drivers.filter((d: any) => isDriverOnlineAndReady(d)).length.toString(), trend: null },
          { label: 'Em Rota Agora', value: drivers.filter((d: any) => d.status === 'on_route' && isDriverOnlineAndReady(d)).length.toString(), trend: null },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 sm:gap-6 p-4 sm:p-6 overflow-y-auto">
          {drivers.map(driver => (
            <div key={driver.id} className="bg-white border border-slate-200 rounded-2xl p-5 hover:border-slate-300 hover:shadow-md transition-all flex flex-col h-full group">
                ...
                <div className="flex items-center gap-3 w-full">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-sm text-amber-500 font-semibold mt-0.5">
                      <Star size={14} fill="currentColor" /> {(typeof driver.rating === 'number' ? driver.rating : 5.0).toFixed(1)}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4">
                {driver.status === 'on_route' && isDriverOnlineAndReady(driver) && (
                ...
                {!isDriverOnlineAndReady(driver) && driver.status !== 'pending_approval' && driver.status !== 'inactive' && (
            ...
            <div className="p-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex items-center gap-1 text-sm text-amber-500 font-semibold">
                      <Star size={16} fill="currentColor" /> {(typeof selectedDriver.rating === 'number' ? selectedDriver.rating : 5.0).toFixed(1)}
                    </div>
                  ...
                  <div className="flex items-center gap-3 text-slate-700">
                    ...
                    <div>
                      ...
                      <div className="font-semibold">
                        {selectedDriver.status === 'on_route' && isDriverOnlineAndReady(selectedDriver) && <span className="text-blue-600">Em rota de entrega</span>}
                        {selectedDriver.status !== 'on_route' && isDriverOnlineAndReady(selectedDriver) && <span className="text-emerald-600">Disponível para rotas (Checklist OK)</span>}
                        {!isDriverOnlineAndReady(selectedDriver) && selectedDriver.status !== 'pending_approval' && selectedDriver.status !== 'inactive' && <span className="text-slate-600">Offline</span>}
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-4">
                ...
                {selectedDriver.status === 'pending_approval' ? (
                  ...
                  <>
                    {(selectedDriver.status === 'active' || selectedDriver.status === 'on_route') ? (
                      <button 
                        onClick={async () => {
                          await syncDriverStatus(
                            { id: selectedDriver.id, name: selectedDriver.name, cpf: selectedDriver.cpf, email: selectedDriver.email },
                            'offline',
                            null
                          );
                          setIsProfileModalOpen(false);
                        }}
                        className="flex-1 px-4 py-2.5 bg-slate-800 text-white hover:bg-slate-900 rounded-xl font-semibold transition-colors shadow-sm flex items-center justify-center gap-2"
                        title="Desconectar / Forçar Offline"
                      >
                        <PowerOff size={18} />
                      ...
                      <button 
                        ...
                        className="flex-1 px-4 py-2.5 bg-emerald-600 text-white hover:bg-emerald-700 rounded-xl font-semibold transition-colors shadow-sm"
                      >
                        Ativar Online
                      </button>
                    )}
