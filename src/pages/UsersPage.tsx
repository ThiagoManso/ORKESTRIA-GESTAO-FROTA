...
import { Check, X, Shield, Edit2, ShieldAlert, UserCheck, AlertCircle, CheckSquare, Square, Trash2 } from 'lucide-react';
import { logCompanyAction } from '../lib/audit';
...
export default function UsersPage({ currentUser }: UsersPageProps) {
  ...
  const handleUpdateStatus = async (userId: string, newStatus: 'approved' | 'rejected') => {
    try {
      ...
      await updateDoc(doc(db, 'system_users', userId), updateData);
      await logCompanyAction(
        newStatus === 'approved' ? 'USER_APPROVED' : 'USER_REJECTED',
        `Alterado status do usuário ID (${userId}) para ${newStatus}`
      );
    } catch (error) {
  ...
  const handleDeleteUser = async (userId: string, userName: string) => {
    if (window.confirm(`Tem certeza que deseja excluir permanentemente o usuário "${userName}" do cadastro?`)) {
      try {
        await deleteDoc(doc(db, 'system_users', userId));
        await logCompanyAction('USER_DELETED', `Usuário ${userName} (ID: ${userId}) foi removido do sistema.`);
      } catch (error) {
  ...
  const handleUpdatePermissions = async (userId: string, newPermissions: ViewState[]) => {
    try {
      const payload: any = { permissions: newPermissions };
      if (currentUser.companyId) {
        payload.companyId = currentUser.companyId;
        if (currentUser.companyName) payload.companyName = currentUser.companyName;
      }
      await updateDoc(doc(db, 'system_users', userId), payload);
      await logCompanyAction(
        'PERMISSIONS_UPDATED',
        `Atualizadas as permissões do usuário ID (${userId}): ${newPermissions.join(', ')}`
      );
      setEditingUser(null);
    } catch (error) {
  ...
  const togglePermission = (permId: ViewState) => {
    ...
    const current = editingUser.permissions || getUserPermissions(editingUser);
                  ...
                  <tr key={user.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {user.status === 'approved' && (
                          <button 
                            onClick={() => setEditingUser({ ...user, permissions: getUserPermissions(user) })}
                        ...
                        <button 
                          ...
                          className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors tooltip flex items-center gap-1 text-sm font-medium"
      ...
      {editingUser && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div>
                ...
                {editingUser.companyName && (
                  <p className="text-xs font-semibold text-brand-cyan mt-0.5">
                    Empresa: {editingUser.companyName}
                  </p>
                )}
              </div>
              <button 
            ...
            <div className="px-6 pt-4 flex items-center justify-between gap-2 border-b border-slate-50 pb-3">
              <button
                type="button"
                onClick={selectAllPermissions}
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-cyan hover:text-cyan-700 transition-colors"
              >
                <CheckSquare size={15} /> Marcar Todas as Telas
              </button>
              <button
                type="button"
